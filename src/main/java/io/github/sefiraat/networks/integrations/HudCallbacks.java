...
public class HudCallbacks {
    ...
    private static String format(ItemStack itemStack, int amount, int limit) {
        ...
        if (meta != null && meta.hasDisplayName()) {
            // ItemMeta.displayName() returns a Component in modern API (adventure).
            // Convert it to a legacy string so ChatUtils.humanize and HUD show nice text.
            Component comp = meta.displayName();
            if (comp != null) {
                itemName = LegacyComponentSerializer.legacySection().serialize(comp);
            } else {
