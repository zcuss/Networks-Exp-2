...
public class NetworkControlV extends NetworkDirectional {
    ...
    private void tryPasteBlock(@Nonnull BlockMenu blockMenu) {
        ...
        // Tambahkan ke cache agar tidak berulang
        this.blockCache.add(targetPosition);

        // Clone lokasi sekarang untuk digunakan di synchronous task
        final Block finalTargetBlock = targetBlock;
        final Material finalMaterial = fetchedStack.getType();

        // Jadwalkan perubahan dunia di main thread
        Bukkit.getScheduler().runTask(Networks.getInstance(), () -> {
            if (finalTargetBlock.getWorld() == null) {
                // jika world null / unload, bersihkan data dan keluar
                this.blockCache.remove(targetPosition);
                return;
            }

            // Set block ke material yang diambil (menggantikan NMS setBlock)
            finalTargetBlock.setType(finalMaterial, false);

            // Force update so clients notice change
            finalTargetBlock.getState().update(true);

            // mcMMO integration: set ineligible jika terpasang
            if (SupportedPluginManager.getInstance().isMcMMO()) {
                mcMMO.getUserBlockTracker().setIneligible(finalTargetBlock);
            }

            // Particle effect di center block
            ParticleUtils.displayParticleRandomly(
                    LocationUtils.centre(finalTargetBlock.getLocation()),
                    Particle.ELECTRIC_SPARK,
                    1,
                    5
            );

            // Done: blockCache tetap berisi posisi sampai next unique tick clears it
        });
    }
