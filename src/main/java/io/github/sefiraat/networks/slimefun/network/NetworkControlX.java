...
public class NetworkControlX extends NetworkDirectional {
    ...
    private void tryBreakBlock(@Nonnull BlockMenu blockMenu) {
        ...
        if (resultStack.getAmount() == 0) {
            // tidak ada ruang => hapus block di main thread menggunakan Bukkit API
            this.blockCache.add(targetPosition);

            final Block finalTarget = targetBlock; // capture for lambda

            Bukkit.getScheduler().runTask(Networks.getInstance(), () -> {
                if (finalTarget.getWorld() == null) {
                    // world unloaded, bersihkan cache dan keluar
                    this.blockCache.remove(targetPosition);
                    return;
                }

                // Set block menjadi AIR (menggantikan NMS setBlock/removeBlockEntity)
                finalTarget.setType(Material.AIR, false);

                // Force update so clients notice (menggantikan engine.checkBlock / sendBlockUpdated)
                finalTarget.getState().update(true);

                // Particle effect di center block
                ParticleUtils.displayParticleRandomly(
                        LocationUtils.centre(finalTarget.getLocation()),
                        1,
                        5,
                        DUST_OPTIONS
                );

                // Kurangi daya dari root
                definition.getNode().getRoot().removeRootPower(REQUIRED_POWER);

                // blockCache tetap berisi posisi sampai next unique tick clears it
            });
        }
    }
