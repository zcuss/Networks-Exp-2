...
public class NetworkQuantumStorage extends SlimefunItem implements DistinctiveItem {
    ...
    // Daftar ID Slimefun yang akan diblokir dari penyimpanan quantum
    private static final Set<String> BLOCKED_SLIMEFUN_IDS = new HashSet<>(Arrays.asList(
            "BASIC_STORAGE",
            "ADVANCED_STORAGE",
            "REINFORCED_STORAGE",
            "VOID_STORAGE",
    ...
    private static boolean isBlacklisted(@Nullable ItemStack itemStack) {
        ...
        // Blokir item dengan durability negatif (non-item?) dan shulker boxes
        if (itemStack.getType().getMaxDurability() < 0) return true;
        if (Tag.SHULKER_BOXES.isTagged(itemStack.getType())) return true;

        // Ambil Slimefun item (bisa null)
        SlimefunItem sfItem = SlimefunItem.getByItem(itemStack);

        // Jangan terima sendiri (jika pemain mencoba menyimpan quantum storage ke dalam quantum storage)
        if (sfItem instanceof NetworkQuantumStorage) return true;

        // Jika item adalah Slimefun dengan ID yang diblokir
        if (sfItem != null && BLOCKED_SLIMEFUN_IDS.contains(sfItem.getId())) {
            return true;
        }
    ...
    private static void updateDisplayItem(@Nonnull BlockMenu menu, @Nonnull QuantumCache cache) {
        if (cache.getItemStack() == null) {
            ...
            // Convert existing lore (Component) or create new
            List<Component> lore = itemMeta != null && itemMeta.lore() != null ? new ArrayList<>(itemMeta.lore()) : new ArrayList<>();

            lore.add(Component.empty());
            lore.add(LegacyComponentSerializer.legacySection().deserialize(
                    Theme.CLICK_INFO + "Voiding: " + Theme.PASSIVE + StringUtils.toTitleCase(String.valueOf(cache.isVoidExcess()))
            ));
            lore.add(LegacyComponentSerializer.legacySection().deserialize(
                    Theme.CLICK_INFO + "Amount: " + Theme.PASSIVE + cache.getAmount()
            ));
            if (cache.supportsCustomMaxAmount()) {
                lore.add(LegacyComponentSerializer.legacySection().deserialize(
                        Theme.CLICK_INFO + "Capacity: " + Theme.SUCCESS + cache.getLimit()
                ));
            }
    ...
    private void onTick(Block block) {
        ...
        if (input != null && input.getType() != Material.AIR) {
            tryInputItem(block.getLocation(), new ItemStack[]{input}, cache);
        }
    ...
    private void setItem(@Nonnull BlockMenu blockMenu, @Nonnull Player player) {
        ...
        if (isBlacklisted(itemStack)) {
            Utils.send(player, Theme.WARNING + "Item ini tidak dapat disimpan di Quantum Storage.");
            return;
        }
    ...
    private QuantumCache createCache(@Nullable ItemStack itemStack, @Nonnull BlockMenu menu, int amount, int maxAmount, boolean voidExcess, boolean supportsCustomMaxAmount) {
        if (itemStack == null || itemStack.getType() == Material.AIR || isDisplayItem(itemStack)) {
            ...
            // Convert lore handling to Component list and remove last 3 lines (legacy behaviour preserved)
            List<Component> lore = itemMeta != null && itemMeta.lore() != null ? new ArrayList<>(itemMeta.lore()) : new ArrayList<>();

            // remove last three lines if present (match old loop behaviour)
            for (int i = 0; i < 3; i++) {
                if (lore.isEmpty()) {
                    break;
                }
