...
public class NetworkVanillaGrabber extends NetworkDirectional {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (stack != null && stack.getType() != Material.AIR) {
                    ...
                    if (Slimefun.getMinecraftVersion().isAtLeast(MinecraftVersion.MINECRAFT_1_20_5)) {
                        // 1.20.5 and above use new API
                        if (potionMeta.getBasePotionType() == PotionType.WATER) {
                            grabItem(blockMenu, stack);
                        }
                    } else {
                        // pre-1.20.5: use helper that suppresses deprecation warning
                        if (isNonWaterOld(potionMeta)) {
                            grabItem(blockMenu, stack);
                            break;
                        }
                    }
    ...
    /**
     * Helper for pre-1.20.5 servers to check base potion data while suppressing
     * deprecation warnings for getBasePotionData()/PotionData (keeps original logic).
     */
    @SuppressWarnings("deprecation")
    private boolean isNonWaterOld(@Nonnull PotionMeta potionMeta) {
        PotionData bpd = potionMeta.getBasePotionData();
        return bpd != null && bpd.getType() != PotionType.WATER;
    }
